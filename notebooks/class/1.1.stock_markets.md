...
Market scale is part of market literacy. A global exchange dashboard can report the same month in several incompatible units: market capitalization in USD, share trading value in USD, listed companies as counts, trades as transaction counts, and derivatives as contracts. For example, the WFE May 2026 dashboard reports market capitalization, value of share trading, listed companies, number of trades, investment flows, and derivatives contract activity as separate indicators, not as interchangeable measures {cite}`wfeDashboardMay2026`. The data extraction notebook uses those fields as a scale snapshot before moving to provider selection.
