...
| --- | --- | --- |
| `DATA_MODE=offline` | Default book build and classroom validation | Uses versioned Banxico and DB.NOMICS snapshots so the book can run without credentials or network access. |
...
3. Download price and macroeconomic data from external providers or read versioned real-data snapshots.
